...
protected:
  ...
  bool createfromFile(const string &file_name, int& maxPI, int& nIsolatedPIs);
  void parseProjection(bool pcnf, ifstream& input_file, char& c);
  // Chang's modification: parse circuit
  void parseCircuit(ifstream& input_file, char& c, int maxPI);
  ...
  // Chang's modification: clsId2ClsOfs[clauseId], clause offset in literal_pool_
  vector<unsigned> clsId2ClsOfs;
  // Chang's modification: simulation results
  ll simRes;
  // Chang's modification: circuit information; 1~MAX_PI, PIs; MAX_PI+1~num_variables()-2, other gates; num_variables()-1, PO
  // fanin0[x], the first fanin of x; fanin1[x], the second fanin of x
  vector<VariableIndex> fanin0, fanin1;
  // faninC0[x], marking the complement information for the first fanin of x; faninC1[x], marking the complement information for the second fanin of x
  vector<uint8_t> faninC0, faninC1;
  // fanouts[x], a vector of fanouts for x
  vector<vector<VariableIndex>> fanouts;
  // Chang's modification: connection between the cnf of the circuit
  vector<VariableIndex> var2ClsOfs;
  vector<ClauseOfs> clsOfs2Var;
  // Chang's modification: pattern[x], 64 simulation patterns of x
  // vector<uint64_t> pattern;
  // vector<uint64_t> pattern_H, pattern_L;
  // vector<bitset<N_BITS>> pattern;
  vector<BitVec> dynPat;

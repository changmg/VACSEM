...
// Chang's modification: print return state
std::ostream & operator << (std::ostream & os, const retStateT state) {
  const std::string stateName[5] = {"EXIT", "RESOLVED", "PROCESS_COMPONENT", "BACKTRACK", "RESTART"};
  std::cout << stateName[(int)(state)];
  return os;
}
...
private:
  ...
  // Chang's modification: special designed conflict resolving for branch 2
  retStateT resolveConflictOnBranch2();
  // Chang's modification: conflict resolving using simulation
  retStateT resolveConflictForSim();
  // Chang's modification: special designed conflict resolving for branch 2
  retStateT resolveConflictForSimOnBranch2();
  // Chang's modification: update clause offset
  void updateClauseOffset();
  // Chang's modification: logic simulation
  // void simulate();
  // void simulateNew();
  void simulateOnce(Component& topComp, BitVec& ALL_ZERO, BitVec& ALL_ONE);
  void simulatePro(std::vector<int> & undecPIs, std::vector<LiteralID> & var2PosLit, Component & topComp);

  /////////////////////////////////////////////
  //  BEGIN small helper functions
  /////////////////////////////////////////////

  // Chang's modification
  // int getUndecPINumInComp() {
  //   auto& topComp = comp_manager_.superComponentOf(stack_.top());
  //   int count = 0;
  //   for (auto itVar = topComp.varsBegin(); *itVar != varsSENTINEL; ++itVar) {
  //     if (*itVar > config_.maxPI)
  //       break;
  //     if (isActive(LiteralID(*itVar, 1))) { // *itVar is unknown
  //       ++count;
  //       if (count > VAR_THRES)
  //         break;
  //     }
  ...
  void getUndecPI(vector<int>& pis) {
    pis.clear();
    auto& topComp = comp_manager_.currentRemainingComponentOf(stack_.top());
    for (auto itVar = topComp.varsBegin(); *itVar != varsSENTINEL; ++itVar) {
      if (*itVar > config_.maxPI)
  ...
  // // Chang's modification: prioritize PIs
  // float scoreOf(VariableIndex v) {
  //   float score = comp_manager_.scoreOf(v);
  //   score += 10.0 * literal(LiteralID(v, true)).activity_score_;
  //   score += 10.0 * literal(LiteralID(v, false)).activity_score_;
  ...
  void printClauses();
  // Chang's modification: print undecided clauses
  void printUndecidedClauses(Component& top); 

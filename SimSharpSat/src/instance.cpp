...
// Chang's modification: record circuit information
void Instance::parseCircuit(ifstream& input_file, char& c, int maxPI) {
  static vector<bool> occurred(num_variables() + 1, false);
  string idstring;
  int gate, lit0, lit1, fi0, fi1, eof;
  if (c == 'c' && input_file >> idstring && idstring == "gate") {
    input_file >> gate;
    assert(gate > 0);
    input_file >> lit0;
    if (lit0 < 0) {
      fi0 = -lit0;
      // faninC0[gate] = numeric_limits<uint64_t>::max();
      faninC0[gate] = 1;
    }
    else {
      fi0 = lit0;
    ...
    if (lit1 < 0) {
      fi1 = -lit1;
      // faninC1[gate] = numeric_limits<uint64_t>::max();
      faninC1[gate] = 1;
    }
    else {
      fi1 = lit1;
    ...
    input_file >> eof;
    assert(fi0 && fi1 && !eof);
    assert(fi0 <= maxPI || occurred[fi0]);
    assert(fi1 <= maxPI || occurred[fi1]);
    fanin0[gate] = fi0, fanin1[gate] = fi1, fanouts[fi0].emplace_back(gate), fanouts[fi1].emplace_back(gate);
    occurred[gate] = true;
  }
}


bool Instance::createfromFile(const string &file_name, int& maxPI, int& nIsolatedPIs) {
  ...
  while (input_file >> c){
    ...
    if (c == 'c' && idstring == "maxPI") {
      input_file >> maxPI;
      cout << "max PI id = " << maxPI << endl;
    }

    if (c == 'c' && idstring == "nIsolatedPIs") {
      input_file >> nIsolatedPIs;
      cout << "nIsolatedPIs = " << nIsolatedPIs << endl;
    }
  ...
  // Chang's modification: circuit information
  fanin0.resize(nVars + 1, 0);
  fanin1.resize(nVars + 1, 0);
  faninC0.resize(nVars + 1, 0);
  faninC1.resize(nVars + 1, 0);
  fanouts.resize(nVars + 1);
  ...
  while ((input_file >> c)) {
    // Chang's modification: do not parse projection
    // parseProjection(pcnf, input_file, c);
    // Chang's modification: parse circuit
    parseCircuit(input_file, c, maxPI);
